...
def load_policy_and_env(stance_leg_fun, q_val, params1, params2):
    ...
    if stance_leg_fun == 1:
        ...
        if q_val == -1:
            torque_dir = -1
            policy_path = _resolve_artifact(__file__, "l1_stand(0,-1)_696.pth")
            actions_fun = [0, -1]
        else:
            torque_dir = 1
            policy_path = _resolve_artifact(__file__, "l1_stand(0,1)_676.pth")
            actions_fun = [0, 1]
    else:
        params_fun = params2
        if q_val == -1:
            ...
            torque_dir = 1
            policy_path = _resolve_artifact(__file__, "l2_stand(0,1)_815.pth")
            actions_fun = [0, 1]
...
with h5py.File(_resolve_artifact(__file__, "working_l1_stand_save_passive_1(-1,0,1)-30,60"), 'r') as h5f:
    Q1 = np.array(h5f['working_save_passive'][:])
with h5py.File(_resolve_artifact(__file__, "working_l2_stand_save_passive_1(-1,0,1)-30,60"), 'r') as h5f:
    Q2 = np.array(h5f['working_save_passive'][:])
with h5py.File(_resolve_artifact(__file__, "reward_check_l1_stand_passive_1(-1,0,1)-30,60"), 'r') as h5f:
    C1 = np.array(h5f['reward_check'][:])
with h5py.File(_resolve_artifact(__file__, "reward_check_l2_stand_passive_1(-1,0,1)-30,60"), 'r') as h5f:
...
#         for k in range(N1):
#             for ll in range(N2):
#                 if Q1[i_,j,k,ll] != 0 and C1[i_,j,k,ll,0] == 1:
#                     print(i_)
#                     print(j)
#                     print(k)
#                     print(ll)
...
_replay = {
    ...
    "case_id": 'flat_1.28_r1',
    "controller": 'passive',
    "seed": int(CMT_REPLAY_SEED),
    "push_off_1_rad_s": 0.51,
    "push_off_2_rad_s": 0.55,
    ...
    "source_fixed_sha256": '4d8d4c938db926963bd52607533cb93142dcc6321ac510c7cb3ef9aea82ea169',
    "old_push_off_1_rad_s": 0.51,
    "old_push_off_2_rad_s": 0.55,
    "search_cmt": 0.11319157587430137,
    "search_energy_J": 3.453770637840483,
    "search_com_displacement_m": 1.3847766581143843,
}
