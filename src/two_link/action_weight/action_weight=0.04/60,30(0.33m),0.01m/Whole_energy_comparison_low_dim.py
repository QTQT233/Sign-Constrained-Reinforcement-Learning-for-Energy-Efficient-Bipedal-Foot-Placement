...
policy_active_continuous.load_state_dict(
    torch.load(MODEL_DIR / 'Policy_Net_Pytorch(-1,0,1)_1348_continuous.pth', map_location=device))
policy_active_continuous.to(device)
policy_passive_discrete.load_state_dict(torch.load(MODEL_DIR / 'Policy_Net_Pytorch_(1,0).pth', map_location=device))
policy_passive_discrete.to(device)
policy_passive_discrete_1.load_state_dict(torch.load(MODEL_DIR / 'Policy_Net_Pytorch(-1,0)_834.pth', map_location=device))
policy_passive_discrete_1.to(device)
policy_active_discrete.load_state_dict(torch.load(MODEL_DIR / 'Policy_Net_Pytorch(-1,0,1)_1613.pth', map_location=device))
