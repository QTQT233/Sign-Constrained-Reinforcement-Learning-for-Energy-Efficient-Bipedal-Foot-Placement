...
policy_active_continuous.load_state_dict(
    torch.load(MODEL_DIR / 'Policy_Net_Pytorch(-1,0,1)_1560_continuous_old_form.pth', map_location=device))
policy_active_continuous.to(device)
policy_passive_discrete.load_state_dict(torch.load(MODEL_DIR / 'Policy_Net_Pytorch(1,0)_900.pth', map_location=device))
policy_passive_discrete.to(device)
policy_passive_discrete_1.load_state_dict(torch.load(MODEL_DIR / 'Policy_Net_Pytorch(-1,0)_1046.pth', map_location=device))
policy_passive_discrete_1.to(device)
policy_active_discrete.load_state_dict(torch.load(MODEL_DIR / 'Policy_Net_Pytorch(-1,0,1)_1635.pth', map_location=device))
