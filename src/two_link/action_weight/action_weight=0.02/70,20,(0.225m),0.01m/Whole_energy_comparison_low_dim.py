...
policy_active_continuous.load_state_dict(
    torch.load(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/'
...
policy_passive_discrete.load_state_dict(torch.load
                                        (f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/'
            ...
            for ll in range(N2):
                ...
                for step in range(steps):
                    ...
                    if (abs(y[0] - theta1_target1) < settle and abs(y[2] - theta2_target1) < settle) or \
                        ...
                        Cmt_save01[i_, j, k, ll] = E / (W * D) if (W * D) > 0 else 0
                        break
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/working_save(0,1)-10-30', 'w') as h5f:
    h5f.create_dataset('working_save', data=working_save01)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Cmt_save(0,1)-10-30', 'w') as h5f:
    h5f.create_dataset('Cmt_save', data=Cmt_save01)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Energy_save(0,1)-10-30', 'w') as h5f:
            ...
            for ll in range(N2):
                ...
                for step in range(steps):
                    ...
                    if (abs(y[0] - theta1_target1) < settle and abs(y[2] - theta2_target1) < settle) or \
                        ...
                        Cmt_save0_1[i_, j, k, ll] = E / (W * D) if (W * D) > 0 else 0
                        break
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/working_save(-1,0)-10-30', 'w') as h5f:
    h5f.create_dataset('working_save', data=working_save0_1)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Cmt_save(-1,0)-10-30', 'w') as h5f:
            ...
            for ll in range(N2):
                ...
                for step in range(steps):
                    ...
                    if (abs(y[0] - theta1_target1) < settle and abs(y[2] - theta2_target1) < settle) or \
                        ...
                        Cmt_save_active_discrete[i_, j, k, ll] = E / (W * D) if (W * D) > 0 else 0
                        break
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/working_save_active_discrete-10-30', 'w') as h5f:
    h5f.create_dataset('working_save_active_discrete', data=working_save_active_discrete)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Energy_save_active_discrete-10-30', 'w') as h5f:
    h5f.create_dataset('Energy_save_active_discrete', data=Energy_save_active_discrete)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Cmt_save_active_discrete(0,1)-10-30', 'w') as h5f:
...
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/working_save_active_continuous-10-30', 'w') as h5f:
    h5f.create_dataset('working_save_active_continuous', data=working_save_active_continuous)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Energy_save_active_continuous-10-30', 'w') as h5f:
    h5f.create_dataset('Energy_save_active_continuous', data=Energy_save_active_continuous)
with h5py.File(f'{ENERGY_COMPARISON_DATA_ROOT}/action_weight=0.02/70,20,(0.225m),0.01m/Cmt_save_active_continuous-10-30', 'w') as h5f:
