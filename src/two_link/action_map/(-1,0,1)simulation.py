...
# policy.load_state_dict(torch.load('D:/L&S/Mas/Project/Walking-RL/result/Policy_Net_Pytorch(-1,0,1).pth'))
policy.load_state_dict(torch.load('C:/Users/Admin/Desktop/paper-code - F/Policy_Net_Pytorch(-1,0,1).pth'))
policy.to(device)
...
check_save = np.zeros((N1, N2, N1, N2, 3))
...
#         for k in range(N1):
#             for ll in range(N2):
#                 if check_save[i_, j, k, ll, 2] > check and working_save[i_, j, k, ll] != -2:
#                     check = check_save[i_, j, k, ll, 2]
#                     save[0] = i_
#                     save[1] = j
#                     save[2] = k
#                     save[3] = ll
...
#                     check_save[i_, j, k, ll, 1] = -1
with h5py.File('C:/Users/Admin/Desktop/paper-code - F/working_save(-1,0,1)-10,30_from2-2', 'w') as h5f:
...
#         plt.imshow(working_save[ii_, :, jj_, :], cmap='jet', origin='lower', vmin=-2, vmax=1,
