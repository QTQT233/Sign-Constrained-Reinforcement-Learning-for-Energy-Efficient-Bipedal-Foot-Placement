# Legacy two-class selector: five additional randomized four-link batches

This provenance component evaluates the frozen V22_3/V9 active PPO controller
against the earlier frozen two-class transition-start selector over five
additional 2,160-case batches. It is retained to reproduce the prior
two-expert analysis and is not the source of the manuscript's current
three-expert values. The
...
## Legacy two-class results

- Active PPO success: 5,143/10,800 (47.620%).
- Two-class selector success: 4,996/10,800 (46.259%).
...
- The two-class selector had lower `Cmt` in 81.818% of both-valid pairs
...
The current three-expert pooled values (4,993 successes, 2,293 both-valid
pairs, and 80.419% lower-`Cmt` pairs) are stored under `three_expert/`.
