...
y = np.zeros(4)
y_old = np.zeros(4)
y[0] = 115.04 * np.pi / 180  # tht1s[19]
y[1] = -1.1684  # dtht1s[11]
y[2] = -33.25 * np.pi / 180  # tht2s[15]
y[3] = -1.8717  # dtht2s[7]
i = np.argmin(abs(y[0] - tht1s))
j = np.argmin(abs(y[1] - dtht1s))
k = np.argmin(abs(y[2] - tht2s))
ll = np.argmin(abs(y[3] - dtht2s))
if Q[i, j, k, ll] == 1 or Q[i, j, k, ll] == 0:
    env = PendulumEnv()
    actions = [0, 1]
...
while not over:
    ...
    plt.plot([l1_draw * np.cos(np.deg2rad(target1)),
              l1_draw * np.cos(np.deg2rad(target1)) + l2_draw * np.sin(np.deg2rad(target2))],
             [l1_draw * np.sin(np.deg2rad(target1)),
              l1_draw * np.sin(np.deg2rad(target1)) - l2_draw * np.cos(np.deg2rad(target2))],
             "k:")
