...
policy1 = torch.nn.Sequential(torch.nn.Linear(4, nnn * 2), torch.nn.Tanh(),
                              ...
                              torch.nn.Linear(nnn, 2), torch.nn.Softmax(dim=1))
policy1.to(device)
optimizer_policy1 = torch.optim.Adam(policy1.parameters(), lr=1e-3)
...
working_save1 = np.zeros((N1, N2, N1, N2))
working_save_passive = np.zeros((N1, N2, N1, N2))
check_save = np.zeros((N1, N2, N1, N2, 5))
check_save1 = np.zeros((N1, N2, N1, N2, 5))
reward_check = np.zeros((N1, N2, N1, N2, 2))
            ...
            for ll in range(N2):
                ...
                for step in range(steps):
                    ...
                    if reward1 or reward2:
                        if reward1:
                            reward_check[i_, j, k, ll, 0] = 1
                            reward_check[i_, j, k, ll, 1] = -1
                        else:
                            reward_check[i_, j, k, ll, 0] = 2
                            reward_check[i_, j, k, ll, 1] = -2
...
for i_ in tqdm(range(N1)):
    for j in range(N2):
        for k in range(N1):
            for ll in range(N2):
                working_save1[i_, j, k, ll] = -2
                check_save1[i_, j, k, ll, 3] = -2
                y = np.array([tht1s[i_], dtht1s[j], tht2s[k], dtht2s[ll]])
                flag = 0
                con = 0
                a_save = 0
                for step in range(steps):
                    ...
                    if reward1 or reward2:
                        if reward1:
                            ...
                            reward_check[i_, j, k, ll, 0] = 2
                            reward_check[i_, j, k, ll, 1] = -2
                        working_save1[i_, j, k, ll] = a_save
                        break
                    inverse_A, B = calc_new_a_b(y, m1, m2, J1, J2, l1, l2, l1_prime, l2_prime, g)
                    state_in_net_ = np.array([(y[0] - np.pi / 2) / rad_theta1_range,
                                              y[1] / speed_range, y[2] / rad_theta2_range, y[3] / speed_range])
                    prob = policy1(torch.FloatTensor(state_in_net_).reshape(1, 4))[0].cpu().detach().numpy()
                    a = actions1[np.argmax(prob)]
                    if a != 0 and check_save1[i_, j, k, ll, 0] == 0:
                        check_save1[i_, j, k, ll, 0] = a
                    if a != 0 and check_save1[i_, j, k, ll, 0] != a:
                        check_save1[i_, j, k, ll, 1] = 1
                        check_save1[i_, j, k, ll, 2] += 1
                    if con == 0:
                        a_save = a
                        con += 1
                    arr = np.array([[-a * torque], [a * torque]])
...
for i in tqdm(range(N1)):
    for j in range(N2):
        for k in range(N1):
            for ll in range(N2):
                if working_save[i, j, k, ll] == -1 and working_save1[i, j, k, ll] != 0:
                    working_save_passive[i, j, k, ll] = -1
                elif working_save[i, j, k, ll] != 0 and working_save1[i, j, k, ll] == 1:
                    working_save_passive[i, j, k, ll] = 1
                elif working_save[i, j, k, ll] == 0 or working_save1[i, j, k, ll] == 0:
                    working_save_passive[i, j, k, ll] = 0
                elif working_save[i, j, k, ll] == -2 or working_save1[i, j, k, ll] == -2:
                    working_save_passive[i, j, k, ll] = -2

with h5py.File('D:/L&S/Mas/Project/Paper2/working_l2_stand_save_passive_1(-1,0,1)-30,60', 'w') as h5f:
    h5f.create_dataset('working_save_passive', data=working_save_passive)
with h5py.File('D:/L&S/Mas/Project/Paper2/reward_check_l2_stand_passive_1(-1,0,1)-30,60', 'w') as h5f:
    h5f.create_dataset('reward_check', data=reward_check)
...
#         plt.imshow(working_save_passive[ii_, :, jj_, :], cmap='jet', origin='lower', vmin=-2, vmax=1,
