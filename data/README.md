...
- The immutable GitHub commit cited in the manuscript is the executable record
  for the submission: source code, repository-hosted checkpoints, simulation
...
- Supplementary Archive S1 contains the complete MPC candidate tables, solver
  traces, validation records, and deterministic replays supplied with the
  journal submission. These files are not part of Zenodo version 1.0.0.
- Supplementary Data S2 is the submission-supplied strict-\(D>0.01\) 12-cell
...
- Supplementary Data S3 combines the frozen ATC-50 table and checksums archived
  ...
  implementation at the cited GitHub commit. S3 does not claim byte-for-byte
  regeneration of the historical table.
- Supplementary Archive S4 contains the 144 transition-level landing records
  and validation, five additional randomized four-link batches, and the
  hold-versus-requery control. A compact mirror is tracked under
  `../supplementary/S4/`; S4 is not part of Zenodo version 1.0.0.

The accepted MPC table is under `../results/paper2_mpc_unified_12case/`.
